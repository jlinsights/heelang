...
function findImageFiles(dir, fileList = []) {
  ...
  files.forEach(file => {
    ...
    } else if (/\.(jpg|jpeg|png)$/i.test(file) && !file.includes('-thumb') && !file.includes('-medium') && !file.includes('-large')) {
...
async function optimizeImage(inputPath, config) {
  ...
  try {
    ...
    console.log(`   크기: ${(originalSize / 1024 / 1024).toFixed(2)}MB → ${(optimizedSize / 1024 / 1024).toFixed(2)}MB (${savings}% 절약)`);
...
async function main() {
  ...
  for (const imagePath of imageFiles) {
    ...
    processedCount++;
    console.log(`   진행률: ${processedCount}/${imageFiles.length} (${(processedCount/imageFiles.length*100).toFixed(1)}%)`);
  }
  
  console.log('\n🎉 이미지 최적화 완료!');
  console.log(`📊 총 절약된 용량: ${((totalOriginalSize - totalOptimizedSize) / 1024 / 1024).toFixed(2)}MB`);
  console.log(`📈 전체 압축률: ${((totalOriginalSize - totalOptimizedSize) / totalOriginalSize * 100).toFixed(1)}%`);
  
  console.log('\n💡 다음 단계:');
  console.log('1. lib/artworks.js에서 이미지 경로를 최적화된 버전으로 업데이트');
